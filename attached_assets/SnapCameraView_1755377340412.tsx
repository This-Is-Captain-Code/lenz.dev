...
export function SnapCameraView({ 
  ...
  useEffect(() => {
    ...
    const initCamera = async () => {
      try {
        ...
        if (currentLens) {
          await applyLensToCanvas(canvasRef.current!, currentLens.id, null);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (canvasRef.current && currentLens && isCameraReady) {
      applyLensToCanvas(canvasRef.current, currentLens.id, null)
        .catch(error => {
      ...
      >
        <div className="bg-background/90 rounded-xl p-4 max-w-full">
          <div className="flex justify-between items-start mb-2">
            <h2 className="text-lg font-bold">{currentLens.name}</h2>
            <Button 
              variant="ghost" 
              size="icon" 
              className="h-6 w-6" 
          ...
          <div className="flex justify-between text-sm">
            <div>
              <span className="font-medium">Creator:</span> {currentLens.creator}
            </div>
            <div>
              <Download className="inline-block h-4 w-4 mr-1" />
              <span>{currentLens.downloads || 0}</span>
            </div>
          </div>
        </div>
      </motion.div>
        ...
        {showControls && isCameraReady && !capturedPhoto && (
          ...
          >
            ...
            <div className="absolute top-0 inset-x-0 p-4 flex justify-between items-start pointer-events-auto">
              ...
              {currentLens && (
                <div className="rounded-full bg-black/20 text-white px-3 py-1 text-sm backdrop-blur-sm">
                  {currentLens.name}
                </div>
              )}
            </div>
